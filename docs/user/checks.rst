...
Checks that a double space is present in translation to avoid false positives on other space-related checks.

The check is false when a double space is found in the source (meaning double space is intentional).
...
(`One apple` instead of `%d apple`). Turn this off by adding a ``strict-format`` flag.
...
opting into XML support or by disabling certain sub-checks.
For example, the following flag enables XML support while
...
single quotes as they have a special meaning. Whenever writing single quote,
it should be written as ``''``.
When not paired, it is treated as beginning of quoting and will not
be shown when rendering the string.
...
:Summary: Vue I18n formatting does not match source
:Scope: translated strings
...
Means a string has been translated already.
This can happen when the translations have been reverted in VCS or lost otherwise.
...
Weblate checks translations of the same string across all translations
within a project to help you keep translations consistent.

The check fails on differing translations of one string within a project.
This can also lead to inconsistencies in displayed checks.
You can find other translations of this string on the
...
.. hint::
   ...
   For performance reasons, the check might not find all inconsistencies,
...
.. hint::
   ...
   using proportional fonts to render the text.
   The :ref:`check-max-size` check does check actual rendering of the text.
...
.. hint::
   ...
   font configured for all strings within a component.
   You can override those values per string in case you need to customize it per string.
...
The presence of colons is also checked for various languages where they do not
...
An ellipsis is usually rendered nicer than three dots in print,
and sounds better with text-to-speech.
...
translation file or defined manually using ``placeholders`` flag,
more can be separated with colons, strings with spaces can be quoted:
...
(exclamation mark, question mark, semicolon and colon).
This rule is used only in a few selected languages like French or Breton,
where space before any double punctuation sign is a typographic rule.
...
The expression is either extracted from the
...
Newlines usually appear in source strings for good reason,
omissions or additions can lead to formatting problems when
the translated text is put to use.
...
A space in the beginning of a string is usually used for indentation
in the interface and thus important to keep.
...
Trailing space is usually utilized to space out neighbouring elements,
so removing it might break the layout.
...
Happens if the source and corresponding translation strings is identical,
down to at least one of the plural forms. Some strings commonly found across all
languages are ignored, and various markup is stripped.
...
consist only of single word which is same in several languages.
This blacklist can be disabled by adding a ``strict-same`` flag to the
string or component.

.. versionchanged:: 4.17

   With ``check-glossary`` flag (see :ref:`check-check-glossary`),
   the untranslatable glossary terms are excluded from the checking.
...
The check is automatically enabled for XML like strings.
You might need to add ``xml-text`` flag in some cases to force turning it on.
...
character references, this check verifies some "good practices", such as
...
attributes, as well as allow *some* custom element or attribute names.
This is partially to ensure that the user is using HTML intentionally.
...
``<a-b>text</a-b>``   no       Custom element tag with matching closing tag.
...
Numerous translations of this string have failing quality checks.
This is usually an indication that something could be done to
improve the source string.

This check failing is quite often caused by a missing full stop at the end of
